...
class PacientesM extends ConexionBD
{
    ...
    if (PacientesM::CheckUser($datosC["usuario"]) == false) {
            ...
            if ($pdo->execute()) {
                PacientesM::email_send222($datosC["usuario"], $datosC["clave"],$datosC["nombre"] .' '.$datosC["apellido"] ,$datosC["correo"]);
                return true;
            }else{
                return false;
            }
...
<a width='100%'; style='background-color: green; color: white;' href=http://localhost/clinica/ingreso-Paciente>Ir a la página</a>
    ...
    private static function email_send222($user, $pass, $nombreApellido, $correo)
    {
        $message = "<html><body>";
        $message .= "<table width='100%' bgcolor='#e0e0e0' cellpadding='0' cellspacing='0' border='0'>";
        $message .= "<tr><td>";
...
<tr>
<td colspan='4' style='padding:15px;'>
<p style='font-size:20px;'>Hola <b>" . $nombreApellido . "</b>. Su registro se ha completado. </p>
<hr />
